...
input$.pipe(
    ...
    mergeMap<string, Observable<GithubUsuariosRespuesta>>(texto => ajax.getJSON( // el texto lo convertimos en una peticion ajax 
...
input$.pipe(
    pluck('target','val'),
    switchMap(texto => ajax.getJSON(url + texto)) // deja pasar la ultima peticion y cancela la demas
